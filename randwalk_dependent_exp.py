"""Reproduce Maei (2011) Fig 5.1, bottom-left panel (Random Walk - Dependent).

Five non-terminal states; 3-D dependent features (insufficient to
represent the true value function exactly).
"""
...
from features import DependentFeatures
from training import ParameterSweep


ETA_VEC = [0.0, 1 / 4, 1 / 2, 1.0, 2.0]
# Thesis Fig 5.1 dependent-features alpha grid.
ALPHA_VEC = [0.5 / 64, 0.5 / 32, 0.5 / 16, 0.5 / 8, 0.5 / 4, 0.5 / 2, 0.5]
...
def main() -> None:
    ...
    episodes = args.episodes if args.episodes is not None else (
        20 if args.quick else THESIS_EPISODES["randwalk_fa"])

    here = os.path.dirname(os.path.abspath(__file__))
    out_dir = args.out_dir or os.path.join(here, "data", "randwalk_fa")
    ...
    print(f"== randwalk_dependent_exp ==  runs={runs}  episodes={episodes}")
