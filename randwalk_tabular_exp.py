"""Reproduce Maei (2011) Fig 5.1, top-left panel (Random Walk - Tabular).

Single thin wrapper over `ParameterSweep`: build the env, the feature
map, and the evaluator; configure the sweep grid; run; save.
"""
...
# Thesis Fig 5.1 sweep grid (random-walk problems).
...
def main() -> None:
    ...
    parser.add_argument("--quick", action="store_true",
                        help="Smoke test (5 runs x 20 episodes).")
    parser.add_argument("--paper", action="store_true",
                        help="Full thesis scale (default).")
    ...
    episodes = args.episodes if args.episodes is not None else (
        20 if args.quick else THESIS_EPISODES["randwalk_tab"])

    here = os.path.dirname(os.path.abspath(__file__))
    out_dir = args.out_dir or os.path.join(here, "data", "randwalk_tab")
    ...
    print(f"== randwalk_tabular_exp ==  runs={runs}  episodes={episodes}")
