"""Plot Random Walk - Tabular features (Fig 5.1 top-left)."""
...
def main() -> None:
    ...
    data_dir = args.data_dir or os.path.join(here, "data", "randwalk_tab")
    fig_dir = args.fig_dir or os.path.join(here, "figures", "randwalk_tab")
    npz_path = os.path.join(data_dir, "results.npz")

    epis_b, T = epis_b_from_npz(npz_path, "randwalk_tab", args.epis_b)
    warn_short_episodes("randwalk_tab", T)

    out = plot_param_study_and_best(
        npz_path=npz_path,
        fig_dir=fig_dir,
        exp_name="randwalkTable",
        epis_b=epis_b,
        x_lim=(0.03, 0.5),
        y_lim=(0.0, 0.16),
        y_lim_curves=(0.0, 0.16),
